...
set devops_ip=172.24.150.130
set ik8s_ip=172.24.148.106
set tk8s_ip=172.24.150.138
set pass=Atl@2022

REM Copy deployment files to the remote server and execute shell scripts
plink -P 22 -no-antispoof -pw "%pass%" user00@%devops_ip% mkdir /mnt/data/tmp_upload_images/tmp_deploy_folder
pscp  -P 22 -pw "%pass%" "*.zip" user00@%devops_ip%:/mnt/data/tmp_upload_images/tmp_deploy_folder/
pscp  -P 22 -pw "%pass%" "*.sh" user00@%devops_ip%:/mnt/data/tmp_upload_images/tmp_deploy_folder/
plink -P 22 -no-antispoof -pw "%pass%" user00@%devops_ip% "cd /mnt/data/tmp_upload_images/tmp_deploy_folder && echo 'Atl@2022' | su -c 'sh _unpack_deploy_images_cmd.sh'"

@REM actually deploy 
plink -P 22 -no-antispoof -pw "%pass%" user00@%devops_ip% "cd /mnt/data/tmp_upload_images/tmp_deploy_folder && echo 'Atl@2022' | su -c 'sh _tag_and_push_image.sh'"
...
plink -P 22 -no-antispoof -pw "%pass%" user00@%devops_ip% "cd /mnt/data/tmp_upload_images/ && mv tmp_deploy_folder trash_can/bes_images_%folder_suffix%"
