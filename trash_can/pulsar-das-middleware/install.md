...
## Prepare Variable
```
RELEASE_NAME=pulsar-das-middleware
NAMESPACE=pulsar-das-middleware

NODEPORT_PROXY=31222
NODEPORT_MANAGER=31223
NODEPORT_GRAFANA=31224
...
## Add Proxy NodePort Service (only for development !!!)
```
cat <<EOF | kubectl apply -f -
apiVersion: v1
kind: Service
...
EOF
```
## Add Grafana NodePort Service (only for development !!!)
```
cat <<EOF | kubectl apply -f -
apiVersion: v1
kind: Service
metadata:
  name: $RELEASE_NAME-pulsar-grafana-nodeport
  namespace: $NAMESPACE
  labels:
    app: pulsar
